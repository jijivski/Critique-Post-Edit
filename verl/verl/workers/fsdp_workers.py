...
class ActorRolloutRefWorker(Worker):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            '''
            len(output)
            all(output.batch['edited_log_probs'][0]==output.batch['edited_log_probs'][4])
                        all(output.batch['edited_log_probs'][0]==output.batch['edited_log_probs'][4])
            False
                        all(output.batch['edited_log_probs'][0]==output.batch['edited_log_probs'][3])
            True
                        all(output.batch['prompts'][0]==output.batch['prompts'][4])
            prompts
            '''
    ...
    def reset_optimizer_state(self):
        """Reset the optimizer state to initial state"""
        try:
            if hasattr(self, 'actor_optimizer') and self.actor_optimizer is not None:
                print(f"[Rank {self.rank}] Resetting actor optimizer state...")
                
                # Safer way to reset optimizer state - avoid direct assignment which might cause serialization issues
                # First, clear all parameter states individually
                if hasattr(self.actor_optimizer, 'state') and self.actor_optimizer.state:
                    # Get all parameter ids first to avoid runtime modification issues
                    param_ids = list(self.actor_optimizer.state.keys())
                    for param_id in param_ids:
                        if param_id in self.actor_optimizer.state:
                            param_state = self.actor_optimizer.state[param_id]
                            # Clear specific tensor states that might cause serialization issues
                            for key in list(param_state.keys()):
                                if isinstance(param_state[key], torch.Tensor):
                                    ...
                                    try:
                                        param_state[key] = param_state[key].cpu()
                                    except:
                                        pass
                                    del param_state[key]
                            # Remove the parameter state entirely
                            del self.actor_optimizer.state[param_id]
                
                # Reset step counter if exists
                if hasattr(self.actor_optimizer, '_step_count'):
                    self.actor_optimizer._step_count = 0
                
                # Clear gradients more safely
                ...
                print(f"[Rank {self.rank}] Actor optimizer state reset successfully")
            else:
                print(f"[Rank {self.rank}] No actor optimizer found to reset")
        except Exception as e:
            print(f"[Rank {self.rank}] Error resetting optimizer state: {e}")
            import traceback
            traceback.print_exc()

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def get_actor_model_state(self):
        """Get actor model state for synchronization."""
        if self._is_actor:
            try:
                ...
                config = FullStateDictConfig(offload_to_cpu=False, rank0_only=True)
                
                # Use the context manager correctly with get_fsdp_state_ctx
                with get_fsdp_state_ctx(self.actor_module_fsdp, StateDictType.FULL_STATE_DICT, config, None):
                    if self.rank == 0:
                        state_dict = self.actor_module_fsdp.state_dict()
                        print(f"[Rank {self.rank}] Successfully collected actor model state dict")
                        return state_dict
                    else:
                        print(f"[Rank {self.rank}] Non-zero rank, returning None")
                        return None
            except Exception as e:
                print(f"[Rank {self.rank}] Error getting actor model state: {e}")
                return None
        else:
            print(f"[Rank {self.rank}] Not an actor worker, returning None")
            return None

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def sync_ref_model_weights(self, actor_state_dict):
        """Synchronize reference model weights with actor state dict.
        Uses FSDP's built-in load function to distribute states across all ranks.
        """
        if self._is_ref:
            try:
                # Use FSDP's built-in load function to distribute states
                if actor_state_dict is not None:
                    from torch.distributed.fsdp import (FullStateDictConfig,
                                                        StateDictType)

                    from verl.utils.fsdp_utils import (
                        fsdp2_load_full_state_dict, get_fsdp_state_ctx)

                    # Check which FSDP strategy is being used
                    if self.config.strategy == "fsdp":
                        # For FSDP v1, use get_fsdp_state_ctx context manager
                        config = FullStateDictConfig(offload_to_cpu=False, rank0_only=False)
                        with get_fsdp_state_ctx(self.ref_module_fsdp, StateDictType.FULL_STATE_DICT, config, None):
                            self.ref_module_fsdp.load_state_dict(actor_state_dict)
                    elif self.config.strategy == "fsdp2":
                        # For FSDP v2, use fsdp2_load_full_state_dict
                        cpu_offload = getattr(self, '_cpu_offload_policy', None)
                        fsdp2_load_full_state_dict(self.ref_module_fsdp, actor_state_dict, self.device_mesh, cpu_offload)
                    else:
                        raise NotImplementedError(f"Unknown FSDP strategy {self.config.strategy}")
                    
                    print(f"[Rank {self.rank}] Reference model weights synchronized successfully")
                else:
                    print(f"[Rank {self.rank}] No state dict provided for synchronization")
            except Exception as e:
                print(f"[Rank {self.rank}] Error synchronizing reference model: {e}")
                raise e
        else:
            print(f"[Rank {self.rank}] Not a reference worker, skipping synchronization")
...
class RewardModelWorker(Worker):
    ...
    def _build_model(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            reward_module = AutoModelForTokenClassification.from_pretrained(pretrained_model_name_or_path=local_path,config=model_config,torch_dtype=torch.bfloat16,attn_implementation="flash_attention_2",trust_remote_code=trust_remote_code,)# num_labels=1,
            # AutoModelForTokenClassification.from_pretrained(pretrained_model_name_or_path=local_path,config=model_config,torch_dtype=torch.bfloat16,attn_implementation="flash_attention_2",trust_remote_code=trust_remote_code,)
            # print('verl/verl/workers/fsdp_workers.py here loaded the reward model, I have added https://github.com/OpenRLHF/OpenRLHF/issues/1039')
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            print("Token level scores shape:", token_level_scores.shape)
            print("Token level scores:", token_level_scores)
            print("========================\n")
