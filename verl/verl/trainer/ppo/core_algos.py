...
def compute_gae_advantage_return_adaptive_lambda_vapo(
    token_level_rewards: torch.Tensor,
    values: torch.Tensor,
    response_mask: torch.Tensor,
    gamma: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        # 计算每个序列的自适应lambda
        lambda_values = 1.0 - 1.0 / (alpha * seq_lengths)
        lambda_values = torch.clamp(lambda_values, min=0.0, max=1.0)  # 确保lambda在[0,1]范围内
        ...
        for b in range(batch_size):
            lastgaelam = 0
            # 使用该batch对应的lambda值
            lambda_b = lambda_values[b]  # 使用计算出的自适应lambda值
            
            for t in reversed(range(gen_len)):# already reversed
                nextvalue = values[b, t + 1] if t < gen_len - 1 else 0.0
                delta = token_level_rewards[b, t] + gamma * nextvalue - values[b, t]
                lastgaelam = delta + gamma * lambda_b * lastgaelam
...
def compute_token_on_off_policy_loss(
    ...
    off_min_clip=None,
    all_max_clip=None, 
    edited_log_probs=None,  # 使用edited_log_probs替代target_probs
    loss_remove_token_mean=False,
    loss_remove_clip=False,
):
    """
    Compute on-policy and off-policy loss using edited log probabilities.
    Adapted from mix_src to use edited_log_probs instead of target_probs.
    ...
    Args:
        ...
        clip_upper_bound: (float) upper bound for clipping
        is_edited_mask: (torch.Tensor) shape: (bs, response_length) - mask for off-policy tokens
        off_cliprange: (float) off-policy clip range
        edited_log_probs: (torch.Tensor) shape: (bs, response_length) - log probs from edited policy
    ...
    # On-policy ratio computation with reshaping options
    ...
    if loss_remove_clip is False:
        ...
        on_pg_loss = verl_F.masked_mean(on_pg_losses, (~is_edited_mask) * eos_mask)
    else:
        on_pg_loss = verl_F.masked_mean(on_pg_losses, (~is_edited_mask) * eos_mask)
        on_pg_clipfrac = torch.tensor(0.0)
    
    # Off-policy loss computation using edited_log_probs

    # Use edited log probabilities for off-policy ratio
    assert edited_log_probs.shape == log_prob.shape
    edited_probs = torch.exp(edited_log_probs)
    off_ratio = torch.exp(log_prob) / (edited_probs + 1e-6)
    off_ratio = off_ratio * is_edited_mask
        
    # Clip off-policy ratio
    if off_max_clip is not None:
        off_ratio = torch.clamp(off_ratio, max=off_max_clip)
        off_ratio_max_clip_frac = verl_F.masked_mean((off_ratio == off_max_clip).float(), is_edited_mask * eos_mask)
    else:
        off_ratio_max_clip_frac = torch.tensor(0.0)
        
    if off_min_clip is not None:
        off_ratio = torch.clamp(off_ratio, min=off_min_clip)
        off_ratio_min_clip_frac = verl_F.masked_mean((off_ratio == off_min_clip).float(), is_edited_mask * eos_mask)
    else:
        off_ratio_min_clip_frac = torch.tensor(0.0)

    off_ratio_mean = verl_F.masked_mean(off_ratio, is_edited_mask * eos_mask)
    if off_ratio_mean.isnan().any().item():
        off_ratio_mean = torch.tensor(0.0)

    # Off-policy loss
    off_pg_losses = -advantages * off_ratio
    off_pg_loss = verl_F.masked_mean(off_pg_losses, is_edited_mask * eos_mask)
    if off_pg_loss.isnan().item() is True:
        off_pg_loss = torch.tensor(0.0)
    off_pg_clipfrac = torch.tensor(0.0)
    
    # Combine on and off policy losses
    is_edited_mask = is_edited_mask.float()
    pg_losses = off_pg_losses * is_edited_mask + on_pg_losses * (1 - is_edited_mask)
    
    # Log on/off probs
    off_policy_probs = torch.exp(log_prob)
    off_policy_prob = verl_F.masked_mean(off_policy_probs, is_edited_mask * eos_mask)
    if off_policy_prob.isnan().item() is True:
        off_policy_prob = torch.tensor(0.0)
    on_policy_probs = torch.exp(old_log_prob)
    on_policy_prob = verl_F.masked_mean(on_policy_probs, (1.0-is_edited_mask) * eos_mask)
...
def compute_policy_loss_vapo_nll(
    old_log_prob,
    log_prob,
    advantages,
    response_mask,
    ...
    clip_ratio_c=3.0,
    loss_agg_mode: str = "token-mean",
    positive_mask=None,  # 新增：正例标识  
    nll_coeff=0.1,       # 新增：NLL损失系数μ  
):
    """Adapted from https://github.com/huggingface/trl/blob/main/trl/trainer/ppo_trainer.py#L1122
    Args:
        old_log_prob: `(torch.Tensor)`
    ...
    Returns:
        ...
        pg_clipfrac_lower: (float)
            the fraction of policy gradient loss being clipped when the advantage is negative
        nll_loss: `a scalar torch.Tensor`
    ...
    # return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower
    # 添加VAPO的NLL损失  
    nll_loss = torch.tensor(0.0, device=log_prob.device)  
    if positive_mask is not None and nll_coeff > 0:  
        # 计算正例的负对数似然损失  
        # token_positive_mask = sequence_positive_mask.unsqueeze(-1).expand_as(response_mask)
        if len(positive_mask.shape) == 1 and positive_mask.shape[0]==log_prob.shape[0]:  # shape is [batch_size] then should be traj level mask
            print(f'using traj level mask')
            positive_mask = positive_mask.unsqueeze(-1)  # turn into [batch_size, 1]
            positive_mask = positive_mask.expand_as(response_mask)
            positive_log_prob = log_prob * positive_mask * response_mask  
            
        else:
            print(f'using token level mask') # should be bsz,seq length
            positive_log_prob = log_prob * positive_mask * response_mask  

        # token_positive_mask
        # nll_loss = -agg_loss(loss_mat=positive_log_prob, loss_mask=token_positive_mask * response_mask, loss_agg_mode=loss_agg_mode)  

        nll_loss = -agg_loss(loss_mat=positive_log_prob, loss_mask=positive_mask * response_mask, loss_agg_mode=loss_agg_mode)  
        # 将NLL损失添加到总损失中  
        pg_loss = pg_loss + nll_coeff * nll_loss  


    print(f'{pg_loss=},{nll_loss=}')
    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, nll_loss  # 返回NLL损失用于监控
