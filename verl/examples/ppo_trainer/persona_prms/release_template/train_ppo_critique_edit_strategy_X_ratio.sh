# verl/examples/release_template/train_ppo_critique_edit.sh
#!/bin/bash
set -ex

# === Parse parameters ===
STRATEGY=${1:-"improve"}  # improve / reward / random
RATIO=${2:-"0.1"}        # edit ratio: 0.0-1.0

# Validate parameters a
if [[ ! "$STRATEGY" =~ ^(improve|reward|random)$ ]]; then
    echo "Error: strategy must be 'improve', 'reward', or 'random'"
    echo "Usage: $0 <strategy> <ratio>"
    echo "  strategy: improve (improvment-based sampling)  / reward (reward-based sampling) / random "
    echo "  ratio: edit ratio between 0.0-1.0 (e.g., 0.1, 0.25, 0.5, 0.75)"
    exit 1
fi
...
case $STRATEGY in
    improve)
        SELECT_FUN="group_rank_by_improvement"
        balance_batch=False
        ;;
    reward)
        SELECT_FUN="sample_edit_reward"
        balance_batch=False
        ;;
    random)
        SELECT_FUN="sample_edit"
        balance_batch=True
        ;;
esac
# some tips: 
# balance_batch maybe need to be examined, sample_edit need to be True, and other two exp are done with balance_batch=False
    # actor_rollout_ref.rollout.select_config.select_fun_args=sample_edit_reward_low_reward \
    # this can used as choose low reward, but still not performing well
...
python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.rollout.edit_config.enable_edit=True \
    actor_rollout_ref.rollout.feedback_config.enable_feedback=True \
    actor_rollout_ref.rollout.select_config.enable_select=True \
